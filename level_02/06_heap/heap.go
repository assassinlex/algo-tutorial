...
func (h *MaxHeap) Push(v int) error {
	...
	h.heapInsert(h.heap, h.size) // 更新数组结构
	h.size++                     // 更新堆大小
	return nil
}
...
func (h *MaxHeap) Pop() (int, error) {
	...
	h.size--                      // 更新堆大小
	array.Swap(h.heap, 0, h.size) // 数组首尾互换
...
func (h *MaxHeap) heapInsert(s []int, i int) {
	for s[i] > s[(i-1)/2] {
		array.Swap(s, i, (i-1)/2)
		i = (i - 1) / 2
	}
}
...
func (h *MaxHeap) heapify(s []int, i, size int) {
	left := i*2 + 1   // 当前节点左子结点索引
	for left < size { // 当前节点有子左节点 [h.size 代表堆中最后一个元素的索引位置]
		largest := left // 默认当前较大的节点索引为左子节点索引
		if left+1 < size && s[left] < s[left+1] {
			largest = left + 1 // 若有子右节点[left+1], 且子右节点值较大
		}
		if s[largest] <= s[i] {
			largest = i // 若当前子节点中较大值已小于父亲节点, 将父亲节点更新, 结束循环
		}
		if largest == i { // 若当前最大子节点就是父亲节点本身, 代表已无后续子节点
			break
		}
		array.Swap(s, largest, i)
		i = largest
		left = i*2 + 1
	}
}
